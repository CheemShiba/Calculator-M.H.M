...
def button_graph():
    try:
        ...
        draw_graph(coefficient)
    except ValueError:
        entry.insert(0, "Invalid Input for Graph")
...
def draw_graph(coefficient):
    try:
        ...
        ax.plot(x, y)
        ax.set_title("Graph of y = mx")
        ax.set_xlabel("x")
        ax.set_ylabel("y")

        # Clearing any previous graphs in the canvas
        for widget in frame_graph.winfo_children():
            widget.destroy()

        canvas = FigureCanvasTkAgg(fig, master=frame_graph)  # Master is correctly set
        canvas.draw()
        ...
        entry.delete(0, END)
        entry.insert(0, "Graph Error: " + str(e))
        print("Error in drawing graph:", e)
...
# Graph Button
button_graph = tk.Button(frame_calculator, text="Graph", padx=85, pady=20, command=button_graph)
button_graph.grid(row=6, column=0, columnspan=3)
